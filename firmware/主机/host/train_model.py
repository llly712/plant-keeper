...
# ============================================================
def main():
    # 检查是否有训练数据 (先看当前目录，再看上级目录)
    data_path = 'data'
    if not os.path.isdir('data'):
        data_path = '../data'
    has_data = os.path.isdir(data_path) and any(
        os.listdir(os.path.join(data_path, d))
        for d in os.listdir(data_path)
        if os.path.isdir(os.path.join(data_path, d))
    )

    if has_data:
        print(f"找到训练数据: {data_path}/")
        train_with_tensorflow(data_path)
    else:
        print("没有训练数据目录 data/")
        print("")
        print("========================================")
...
def train_with_tensorflow(data_path='data'):
    """用TensorFlow训练模型"""
    try:
